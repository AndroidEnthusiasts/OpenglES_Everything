...
public class GLESUtils {
     ...
     * @param version es支持的版本
     * @return shaderObjectId
     */
    public static int compileShaderCode(int type, String shaderCode,int version) {
        //得到一个着色器的ID。主要是对ID进行操作
        if (GLES_VERSION_2 == version) {
            ...
            return shaderObjectId;
        }else if(GLES_VERSION_3 == version){
            ...
            if (shaderObjectId != 0) {
                ...
                int[] status = new int[1];
                //调用getShaderIv ，传入GL_COMPILE_STATUS进行查询
                GLES30.glGetShaderiv(shaderObjectId, GLES30.GL_COMPILE_STATUS, status, 0);
